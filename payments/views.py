...
from util.payment import process_payment_helper
...
def remove_card(request, stripe_card_id):
    ...
    if 'POST' != request.method or stripe_card.stripe_customer.user.id != request.user.id:
...
@login_required
def process_payment(request, card_id=None):
    payment_intent_id = card_id

    return process_payment_helper(request, payment_intent_id)


@login_required
def process_3d_secure(request):
